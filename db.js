...
//  Actúa como caché rápida de la base de datos en la nube.
...
const DB = {
  ...
  async getLotByUid(uid) {
    if (!uid) return null;
    const all = await this.getLots();
    return all.find(l => l.lot_uid === uid) || null;
  },
  ...
  // Opera en centavos enteros para evitar error de punto flotante acumulado.
  // Trunca (no redondea) el costo por fracción para no inflar el COGS.
  async consumeFIFO(pid, unitsNeeded) {
    const lots     = await this.getActiveLots(pid);
    let   rem      = unitsNeeded;
    let   cogsCents = 0;   // acumulador en centavos

    for (const lot of lots) {
      if (rem <= 0) break;
      const take         = Math.min(lot.qty_remaining, rem);
      // Truncar costo unitario a centavos enteros — nunca redondear hacia arriba en deducción
      const costCentsU   = Math.floor(lot.cost * 100);
      cogsCents         += costCentsU * take;
      lot.qty_remaining -= take;
      rem               -= take;
      await this.saveLot(lot);
    }
    // Fallback si no había lotes suficientes
    if (rem > 0) {
      const p = await this.getProduct(pid);
      cogsCents += Math.floor((p?.cost || 0) * 100) * rem;
    }
    await this.updateStock(pid, -unitsNeeded);
    return cogsCents / 100;   // devolver en Bs con 2 decimales implícitos
  },
  ...
  async getStats(sales) {
    ...
    for (const s of sales) {
      const items = s.items || [];
      if (items.length) {
        // Derivar totales desde los ítems (fuente real — resiste cambios de ID)
        for (const item of items) {
          const lt = item.lineTotal || 0;
          const ic = item.item_cogs || 0;
          const iu = item.units     || 0;
          revenue += lt;
          cogs    += ic;
          units   += iu;
          // Agrupar por NOMBRE (no por ID) → misma cerveza = misma fila siempre
          const key = (item.product_name || '—').toLowerCase().trim();
          if (!byProd[key]) byProd[key] = { name: item.product_name || '—', revenue: 0, cogs: 0, units: 0 };
          byProd[key].revenue += lt;
          byProd[key].cogs    += ic;
          byProd[key].units   += iu;
        }
      } else {
        // Fallback: venta sin detalle de ítems (datos muy viejos)
        revenue += s.total      || 0;
        cogs    += s.total_cogs || 0;
      }
    }
