...
function initScan() {
  ...
  async function processScan(val) {
    ...
    if (found) {
      ...
      setTimeout(() => input.focus(), 80); // refoco inmediato para siguiente escaneo
      return;
    }
...
function initPayment() {
  ...
  // Inicializar fecha de venta con hora actual
  function updateVentaFecha() {
    const now = new Date();
    const local = new Date(now.getTime() - now.getTimezoneOffset() * 60000);
    document.getElementById('ventaFecha').value = local.toISOString().slice(0, 16);
  }
  updateVentaFecha();

  // Botón "Ahora" para poner fecha/hora actual
...
async function processSale() {
  ...
  // Guardar en base de datos y esperar confirmación
  toast('Registrando venta en base de datos…', 'success');
  try {
    await Sheets.addVenta(sale);
    await DB.saveSale({ ...sale, synced: true });
    
    // Actualizar productos vendidos en base de datos
    const updatePromises = saleItems.map(item => 
      DB.getProduct(item.product_id).then(up => {
        if (up) return Sheets.saveProduct(up, getUser().email);
      })
    );
    await Promise.all(updatePromises);
    await Sheets.syncLotes();
    
    toast(`Venta registrada y sincronizada · ${fmt(total)}`, 'success');
  } catch (err) {
    toast('Venta guardada localmente (sin conexión a la base de datos)', 'warn');
...
async function renderProducts(filter = '') {
  ...
  if (!items.length) {
    grid.innerHTML = `<div style="padding:40px;text-align:center;color:var(--text3)">
      Sin productos. Crea un producto nuevo o sincroniza con la base de datos.</div>`;
    return;
  }
...
async function saveProduct() {
  ...
  // Guardar en IndexedDB primero (rápido)
  await DB.saveProduct(prod);
  toast(editProdId ? 'Guardando en base de datos…' : 'Creando en base de datos…', 'success');
  
  // Guardar en base de datos y esperar confirmación
  try {
    await Sheets.saveProduct(prod, getUser().email);
    closeModal('modalProducto');
    toast(editProdId ? 'Producto actualizado y sincronizado' : 'Producto creado y sincronizado', 'success');
  } catch (err) {
    closeModal('modalProducto');
    toast('Guardado localmente (sin conexión a la base de datos)', 'warn');
...
function confirmDelete(id) {
  DB.getProduct(id).then(p => {
    ...
    // Apuntar al clon — el original fue removido del DOM
    const bar  = nb.querySelector('.btn-hold-progress');
    const lbl  = nb.querySelector('.btn-hold-label');
    const HOLD = 4000;
...
async function openStockAdj(id = null) {
  ...
  document.getElementById('adjFecha').value    = new Date().toISOString().split('T')[0]; // hoy
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    await DB.addLot({ product_id: adjProdId, date: fecha, cost: costo, qty_initial: qty, qty_remaining: qty, notes: nota });
    const mov = { product_id: adjProdId, product_name: p.name, date: fecha, type: 'entrada', qty, notes: `Lote FIFO · Bs ${costo}/u · ${nota}`, usuario: getUser().email };
    await DB.addMovement(mov);
    
    // Sincronizar con base de datos y esperar confirmación
    toast('Guardando entrada en base de datos…', 'success');
    try {
      await Promise.all([
        Sheets.addMovimiento(mov),
        Sheets.saveProduct(p, getUser().email),
        Sheets.syncLotes()
      ]);
      closeModal('modalStock');
      toast(`Lote creado y sincronizado: ${qty} uds. a Bs ${costo}/u`, 'success');
    } catch (err) {
      closeModal('modalStock');
      toast('Guardado localmente (sin conexión a la base de datos)', 'warn');
      console.warn('[saveStockAdj] Error al sincronizar:', err);
    }
  } else {
    const delta = tipo === 'ajuste_neg' ? -qty : qty;
    await DB.updateStock(adjProdId, delta);
    const mov = { product_id: adjProdId, product_name: p.name, date: fecha, type: tipo, qty: delta, notes: nota, usuario: getUser().email };
    await DB.addMovement(mov);
    
    // Sincronizar con base de datos
    toast('Guardando ajuste en base de datos…', 'success');
    try {
      await Promise.all([
        Sheets.addMovimiento(mov),
        Sheets.saveProduct(p, getUser().email)
      ...
      closeModal('modalStock');
      toast('Stock ajustado y sincronizado', 'success');
    } catch (err) {
      closeModal('modalStock');
      toast('Guardado localmente (sin conexión a la base de datos)', 'warn');
      console.warn('[saveStockAdj] Error al sincronizar:', err);
...
async function printReport() {
  ...
  const allLots  = (await DB.getLots()).filter(l => l.qty_remaining > 0)
  ...
  }).join('') || '<tr><td colspan="5" style="text-align:center;color:#999;padding:12px">Sin lotes activos</td></tr>';
...
function getUser() {
  return {
    email: localStorage.getItem('pos_user_email') || 'sistema',
    name:  localStorage.getItem('pos_user_name')  || 'Sistema'
  };
}

// ═══════════════════════════════════════════════════════
//  AUTH — Login con Google (una sola vez)
// ═══════════════════════════════════════════════════════
function showUserChip(name, email) {
...
function logout() {
  ...
  console.log('[LOGOUT] Cerrando sesión - limpiando credenciales');
  localStorage.removeItem('pos_user_email');
  localStorage.removeItem('pos_user_name');
  console.log('[LOGOUT] Credenciales eliminadas - recargando');
  location.reload();
}
...
function setupLoginScreen() {
  ...
  // Solo mostrar botón de login, esperar acción explícita del usuario
  document.getElementById('btnGoogleLogin').addEventListener('click', () => {
    const btn   = document.getElementById('btnGoogleLogin');
    const errEl = document.getElementById('loginError');
    btn.disabled = true; btn.textContent = 'Conectando…';
    if (errEl) errEl.textContent = '';
    console.log('[LOGIN] Usuario hizo clic en login - solicitando auth explícita');
    // Usuario hizo clic explícitamente → mostrar popup de selección de cuenta
    Sheets.requestAuth(handleOAuthUser);
  });
  
  // Intento silencioso por si ya consintió antes (sin popup)
  // IMPORTANTE: No pasar callback aquí para evitar que se llame handleOAuthUser
  // después de que el usuario ya está logueado
  console.log('[LOGIN] Intentando auth silenciosa inicial');
  Sheets.initAuth(handleOAuthUser);
}

async function handleOAuthUser(info) {
  console.log('[HANDLE_OAUTH] Callback llamado con info:', info);
  
  // Verificar si ya hay sesión - CRUCIAL para evitar loops
  const savedEmail = localStorage.getItem('pos_user_email');
  if (savedEmail) {
    console.log('[HANDLE_OAUTH] Sesión ya existe:', savedEmail, '- SALIENDO del callback');
  ...
  if (!info?.email) {
    console.warn('[HANDLE_OAUTH] No se obtuvo email');
    // Solo mostrar error si el botón existe (usuario intentó login manualmente)
    if (errEl) errEl.textContent = 'No se pudo obtener el correo. Intenta de nuevo.';
    if (btn) { btn.disabled = false; btn.textContent = 'Entrar con Google'; }
    return;
  }
  
  console.log('[HANDLE_OAUTH] Verificando acceso para:', info.email);
  if (btn) { btn.disabled = true; btn.textContent = 'Verificando…'; }
  const allowed = await Sheets.checkUserAccess(info.email);
  if (!allowed) {
    console.warn('[HANDLE_OAUTH] Acceso denegado para:', info.email);
    if (errEl) errEl.textContent = `${info.email} no tiene acceso. Contacta al administrador.`;
    if (btn) { btn.disabled = false; btn.textContent = 'Entrar con Google'; }
    return;
  }
  
  // Guardar credenciales y recargar
  console.log('[HANDLE_OAUTH] Acceso concedido - guardando credenciales');
  localStorage.setItem('pos_user_email', info.email);
  localStorage.setItem('pos_user_name',  info.name || info.email);
  console.log('[HANDLE_OAUTH] Credenciales guardadas - recargando página');
  location.reload();
}
...
async function init() {
  ...
  console.log('[INIT] Verificando sesión - savedEmail:', savedEmail);

  if (!savedEmail) {
    console.log('[INIT] No hay sesión - mostrando login screen');
    setupLoginScreen();
    return;
  }

  console.log('[INIT] Sesión activa - iniciando app');
  // Sesión activa — arrancar directamente sin pedir correo
  ...
  // Token silencioso para escritura (ya está identificado, sin popup)
  // Solo inicializar si hay usuario logueado
  if (localStorage.getItem('pos_user_email')) {
    console.log('[INIT] Usuario logueado:', localStorage.getItem('pos_user_email'));
    Sheets.initAuth();
    console.log('[INIT] Sheets.initAuth() llamado - esperando token silencioso');
  }
  ...
  async function syncFromSheets(silent = false) {
    ...
    try {
      await Sheets.loadFromSheets();
      setStatus('ok', 'Base de datos lista');
      renderProducts(); renderInventory();
      syncCount++;
      // Mostrar notificación solo en el primer sync automático
      if (syncCount === 1 && silent) {
        console.log('[Auto-sync] Datos actualizados correctamente');
      }
    } catch (err) {
  ...
  // Auto-sync cada 90 segundos para mantener datos actualizados
  setInterval(() => syncFromSheets(true), 90 * 1000);

  // Sync de ventas pendientes cada 30 segundos (sin internet al vender)
  setInterval(async () => {
    if (!Sheets.isConfigured() || !Sheets._token) return;
    const pendientes = (await DB.getSales()).filter(s => !s.synced);
    if (pendientes.length) {
      console.log(`[Auto-sync] Enviando ${pendientes.length} venta(s) pendiente(s)...`);
      for (const s of pendientes) {
        try {
          await Sheets.addVenta(s);
          await DB.saveSale({ ...s, synced: true });
          toast('Ventas sincronizadas', 'success');
        } catch (e) {
          console.warn('[Auto-sync] Error al enviar venta:', e);
        }
      }
    }
  }, 30 * 1000);
