...
function setupLoginScreen() {
  ...
  const initSignIn = () => {
    google.accounts.id.initialize({
      client_id:             (typeof SHEETS_CLIENT_ID !== 'undefined' ? SHEETS_CLIENT_ID : ''),
  ...
  if (typeof google !== 'undefined') initSignIn();
  else window.addEventListener('gsi-loaded', initSignIn, { once: true });
}
...
async function handleGoogleCredential(response) {
  ...
  try {
    ...
    // Consultar hoja Usuarios en Google Sheets
    const allowed = await Sheets.checkUserAccess(email);
    if (!allowed) {
      if (errEl) errEl.textContent = `${email} no tiene acceso. Contacta al administrador.`;
      if (btnLogin) { btnLogin.disabled = false; btnLogin.innerHTML = '&#9654; Entrar con Google'; }
      return;
    }

    localStorage.setItem('pos_user_email', email);
