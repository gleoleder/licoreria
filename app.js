...
async function renderInventory(filter = '') {
  ...
  // Precalcular vencimiento próximo en cada producto (para ordenar)
  for (const p of deduped) {
    const lots = await DB.getLotsByProduct(p.id);
    const activos = lots.filter(l => l.qty_remaining > 0 && l.expiry);
    if (activos.length) {
      const min = activos.reduce((m, l) => new Date(l.expiry) < new Date(m.expiry) ? l : m);
      p._proxExpiry = new Date(min.expiry).getTime();
      p._proxExpiryIso = min.expiry;
    } else {
      p._proxExpiry    = Infinity;
      p._proxExpiryIso = null;
    }
  }
  ...
  for (const p of items) {
    // Usar el vencimiento precalculado arriba — no releer lotes
    const proxVencIso  = p._proxExpiryIso || null;
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    // IndexedDB listo — cerrar modal y mostrar éxito inmediatamente
    closeModal('modalStock');
    toast(`Lote guardado · ${qty} ud. a Bs ${costo.toFixed(4)}/u`, 'success');
    updateExpiryBadge();
    renderInventory(document.getElementById('invSearch').value);

    // Enviar a Sheets en background — sin bloquear la UI
    // addLote en Apps Script ya verifica duplicados por lot_uid
    Sheets.addLote(newLot, p.name)
      .then(() => Sheets.addMovimiento(mov))
      .then(() => Sheets.saveProduct(pActualizado, getUser().email))
      .then(() => { DB.saveLot({ ...newLot, synced: true }); setStatus('ok', 'Lote en BD'); })
    ...
    Sheets.addMovimiento(mov)
      .then(() => Sheets.saveProduct(pActualizado, getUser().email))
      .then(() => setStatus('ok', 'Ajuste en BD'))
      .catch(() => setStatus('error', 'Sin conexión — guardado local'));
  }
}
