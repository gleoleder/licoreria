...
async function processSale() {
  ...
  try {
    ...
    showOverlay('Actualizando inventario…');
    for (const item of saleItems) {
      const lots = await DB.getLotsByProduct(item.product_id);
      for (const lot of lots) {
        await Sheets.updateLote(lot, item.product_name);
      }
      const up = await DB.getProduct(item.product_id);
      if (up) await Sheets.saveProduct(up, getUser().email);
    }
    ...
    hideOverlay();
    // Sin conexión: mostrar ticket igual y limpiar carrito
    showTicket(sale);
...
async function loadAdjProduct(pid) {
  ...
  if (info.lots.length) {
    prev.innerHTML = `<div class="lot-list-title">Lotes activos (FIFO)</div>` +
      info.lots.map((l,i) => `<div class="lot-row ${i===0?'oldest':''}" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        <span class="lot-num">#${i+1}</span>
        <span class="lot-cost">Bs ${l.cost.toFixed(2)}/u</span>
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    // Costo unitario calculado con precisión de centavos
    const costo = calcCostoUnitario(costoTotal, qty);
    const lotUid = generarLoteId();
    const lotId  = await DB.addLot({ lot_uid: lotUid, product_id: adjProdId, date: fecha, cost: costo, cost_total: costoTotal, qty_initial: qty, qty_remaining: qty, notes: nota });
    const newLot = await DB.getLot(lotId);
    const pActualizado = await DB.getProduct(adjProdId);
    const mov = { product_id: adjProdId, product_name: p.name, date: fecha, type: 'entrada', qty, notes: `Lote FIFO · Total Bs ${costoTotal} · Bs ${costo.toFixed(4)}/u · ${nota}`, usuario: getUser().email };
    await DB.addMovement(mov);
