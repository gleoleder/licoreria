...
async function updateExpiryBadge() {
  ...
  const count = lots.filter(l =>
    l.expiry && l.qty_remaining > 0 && daysToExpiry(l.expiry) <= EXPIRY_WARN_DAYS
  ).length;
...
async function processSale() {
  ...
  try {
    ...
    // Actualizar lotes e inventario en background — no bloquea el ticket
    Promise.all(saleItems.flatMap(item => [
      DB.getLotsByProduct(item.product_id).then(lots =>
        Promise.all(lots.map(lot => Sheets.updateLote(lot, item.product_name)))
      ),
      DB.getProduct(item.product_id).then(up => up && Sheets.saveProduct(up, getUser().email))
    ])).catch(err => console.warn('[processSale] Sync inventario:', err));
    ...
    hideOverlay();
    // Sin conexión o timeout: mostrar ticket igual y limpiar carrito
    showTicket(sale);
...
async function saveProduct() {
  ...
  Sheets.saveProduct(prod, getUser().email)
    .then(() => setStatus('ok', 'Producto sincronizado'))
    .catch(err => {
...
async function renderInventory(filter = '') {
  ...
  for (const p of items) {
    ...
    // Leer lotes para mostrar vencimiento más próximo
    const lots = await DB.getLotsByProduct(p.id);
    const lotsActivos = lots.filter(l => l.qty_remaining > 0 && l.expiry);
    // El lote más próximo a vencer (o ya vencido) entre los activos
    const proxVenc = lotsActivos.length
      ? lotsActivos.reduce((min, l) => daysToExpiry(l.expiry) < daysToExpiry(min.expiry) ? l : min)
      : null;
    const expiryDays = proxVenc ? daysToExpiry(proxVenc.expiry) : null;
    const expiryAlerta = expiryDays !== null && expiryDays <= EXPIRY_WARN_DAYS;

    const sig = `${p.name}|${p.stock}|${p.min_stock}|${p.category}|${p.base_unit}|${proxVenc?.expiry||''}`;
    ...
    if (tr.dataset.sig !== sig) {
      ...
      tr.innerHTML = `
        ...
        <td>${proxVenc ? expiryTag(proxVenc.expiry) : '<span style="color:var(--text3);font-size:.75rem">—</span>'}</td>
        <td>
...
async function loadAdjProduct(pid) {
  ...
  // Mostrar último costo unitario como referencia (solo lectura)
    ...
    prev.innerHTML = `<div class="lot-list-title">Lotes activos (FIFO)</div>` +
      info.lots.map((l,i) => {
        ...
        return `<div class="lot-row ${i===0?'oldest':''}" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          ...
          ${exp}
          <button class="btn-edit-lot" data-lot-id="${l.id}" data-prod-id="${pid}" title="Editar lote" style="margin-left:auto">✏</button>
          <button class="btn-del-lot"  data-lot-id="${l.id}" data-prod-id="${pid}" title="Eliminar lote">🗑</button>
        </div>`;
      }).join('') +
      `<div class="lot-notice">Nuevo lote se creará independiente con el costo que ingreses.</div>`;
    prev.querySelectorAll('.btn-edit-lot').forEach(btn => {
      btn.addEventListener('click', () => openEditLote(parseInt(btn.dataset.lotId), parseInt(btn.dataset.prodId), () => loadAdjProduct(pid)));
    });
    prev.querySelectorAll('.btn-del-lot').forEach(btn => {
      btn.addEventListener('click', () => deleteLote(parseInt(btn.dataset.lotId), parseInt(btn.dataset.prodId)));
    });
  } else {
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    const newLot = await DB.getLot(lotId);
    const pActualizado = await DB.getProduct(adjProdId);
    const mov = { product_id: adjProdId, product_name: p.name, date: fecha, type: 'entrada', qty, notes: `Lote FIFO · Total Bs ${costoTotal} · Bs ${costo.toFixed(4)}/u · ${nota}`, usuario: getUser().email, synced: false };
    await DB.addMovement(mov);

    closeModal('modalStock');
    toast(`Lote guardado · ${qty} ud. a Bs ${costo.toFixed(4)}/u`, 'success');
    updateExpiryBadge();
    renderInventory(document.getElementById('invSearch').value);

    // Intentar subir a Sheets ahora — si falla, el auto-retry lo reintentará
    Promise.all([
      Sheets.addMovimiento(mov),
      Sheets.saveProduct(pActualizado, getUser().email),
      Sheets.addLote(newLot, p.name)
    ]).then(async () => {
      await DB.saveLot({ ...newLot, synced: true });
      setStatus('ok', 'Lote sincronizado');
    }).catch(err => {
      setStatus('error', 'Sin conexión — se subirá al reconectar');
      console.warn('[saveStockAdj] Error al sincronizar lote:', err);
    });
    ...
    const pActualizado = await DB.getProduct(adjProdId);
    const mov = { product_id: adjProdId, product_name: p.name, date: fecha, type: tipo, qty: delta, notes: nota, usuario: getUser().email, synced: false };
    await DB.addMovement(mov);
    ...
    Promise.all([
      ...
      setStatus('ok', 'Ajuste sincronizado');
    }).catch(err => {
      setStatus('error', 'Sin conexión — se subirá al reconectar');
      console.warn('[saveStockAdj] Error al sincronizar ajuste:', err);
    });
  }
}
...
async function saveEditLote() {
  const lotId  = parseInt(document.getElementById('elLoteId').value);
  const prodId = parseInt(document.getElementById('elProdId').value);
  ...
  await DB.saveLot(updated);
  await DB.recalcStock(prodId);

  // Sincronizar con Sheets en background
  const prod = await DB.getProduct(prodId);
  Promise.all([
    Sheets.editLote(updated, prod?.name || ''),
    prod ? Sheets.saveProduct(prod, getUser().email) : Promise.resolve()
  ]).then(async () => {
    await DB.saveLot({ ...updated, synced: true });
  ...
  updateExpiryBadge();
  if (_editLoteCallback) _editLoteCallback();
}
...
async function deleteLote(lotId, productId) {
  ...
  try {
    ...
    updateExpiryBadge();
    await loadAdjProduct(productId);
    renderInventory(document.getElementById('invSearch').value);
  } catch (err) {
    hideOverlay();
...
async function init() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') ['modalProducto','modalStock','modalTicket','modalScanner','modalDiscount','modalDelete','modalEditVenta','modalEditLote'].forEach(closeModal);
  });
  ...
  async function syncFromSheets(silent = false) {
    ...
    try {
      ...
      refreshCats(); // actualizar categorías desde localStorage recién escrito
  ...
  // ── Auto-retry de pendientes cada 45 segundos ─────────
  // Sube a Sheets todo lo que quedó sin sincronizar (ventas, lotes, movimientos)
  // IndexedDB nunca se borra — es el caché local permanente para modo offline
  setInterval(async () => {
    if (!Sheets.isConfigured()) return;

    // 1. Ventas sin sincronizar
    const ventasPend = (await DB.getSales()).filter(s => !s.synced);
    for (const s of ventasPend) {
      try {
        await Sheets.addVenta(s);
        await DB.saveSale({ ...s, synced: true });
        toast('✓ Venta sincronizada', 'success');
      } catch (e) {
        console.warn('[Auto-sync] venta pendiente:', e);
        break; // si falla, esperar al próximo ciclo
      }
    }

    // 2. Lotes sin sincronizar
    const lotesPend = (await DB.getLots()).filter(l => !l.synced);
    for (const l of lotesPend) {
      try {
        const prod = await DB.getProduct(l.product_id);
        if (!prod) continue;
        await Sheets.addLote(l, prod.name);
        await Sheets.saveProduct(prod, getUser().email);
        await DB.saveLot({ ...l, synced: true });
        toast('✓ Lote sincronizado', 'success');
      } catch (e) {
        console.warn('[Auto-sync] lote pendiente:', e);
        break;
      }
    }

    // 3. Movimientos sin sincronizar
    const movsPend = (await DB.getMovements()).filter(m => !m.synced);
    for (const m of movsPend) {
      try {
        await Sheets.addMovimiento(m);
        // Marcar como sincronizado — movements usa _put con id
        await DB._put('movements', { ...m, synced: true });
        toast('✓ Movimiento sincronizado', 'success');
      } catch (e) {
        console.warn('[Auto-sync] movimiento pendiente:', e);
        break;
      }
    }
