  ...
  if (info.lots.length > 0) {
    ...
    const lines = info.lots.map((l, i) => {
      ...
      return `<div class="lot-row ${i===0?'oldest':''}" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ...
        <button class="btn-del-lot" data-lot-id="${l.id}" data-prod-id="${id}" title="Eliminar lote" style="margin-left:auto">🗑</button>
      </div>`;
    }).join('');
    ...
    fpLotesEl.querySelectorAll('.btn-del-lot').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        openEditProduct(id); // refrescar modal de producto
      });
    });
    ...
    prev.innerHTML = `<div class="lot-list-title">Lotes activos (FIFO)</div>` +
      info.lots.map((l,i) => {
        ...
        return `<div class="lot-row ${i===0?'oldest':''}" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          ...
          ${exp}
          <button class="btn-del-lot" data-lot-id="${l.id}" data-prod-id="${pid}" title="Eliminar este lote" style="margin-left:auto">🗑</button>
        </div>`;
      }).join('') +
      `<div class="lot-notice">Nuevo lote se creará independiente con el costo que ingreses.</div>`;
    // Agregar listeners de eliminar lote
...
async function init() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') ['modalProducto','modalStock','modalTicket','modalScanner','modalDiscount','modalDelete','modalEditVenta'].forEach(closeModal);
  });
  ...
  // Revisar vencimientos al cargar (con delay para no chocar con otros toasts)
