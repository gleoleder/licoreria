...
async function renderInventory(filter = '') {
  ...
  // Deduplicar por nombre — segunda defensa contra duplicados en Sheets
  ...
  let items = filter
    ? deduped.filter(p => p.name.toLowerCase().includes(filter.toLowerCase()))
    : deduped;
  ...
  // Limpiar siempre — evita duplicados cuando loadFromSheets reasigna IDs locales
  tbody.innerHTML = '';
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    // Subir a Sheets — con await para evitar envíos duplicados si el usuario
    // guarda varios lotes seguidos antes de que resuelva el anterior
    showOverlay('Guardando en base de datos…');
    try {
      await Sheets.addLote(newLot, p.name);
      await Sheets.addMovimiento(mov);
      await Sheets.saveProduct(pActualizado, getUser().email);
      await DB.saveLot({ ...newLot, synced: true });
      setStatus('ok', 'Lote sincronizado');
    } catch (err) {
      setStatus('error', 'Sin conexión — guardado solo localmente');
