...
async function openEditProductByName(nombre) {
  ...
  if (!p) { toast('Producto no encontrado — actualiza el inventario', 'warn'); return; }
...
function renderInventory(filter = '') {
  ...
  for (const p of prods) {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-icon" onclick="openStockAdjByName(${JSON.stringify(p.name)})">+ Stock</button>
        <button class="btn-icon" style="margin-left:4px" onclick="openEditProductByName(${JSON.stringify(p.name)})">✏</button>
      </td>`;
    frag.appendChild(tr);
  }
...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    toast(`Lote guardado · ${qty} ud. · Stock total: ${nuevoStock}`, 'success');
    updateExpiryBadge();
    renderInventory(document.getElementById('invSearch').value);

    // 4. Enviar lote y movimiento a Sheets
    ...
    closeModal('modalStock');
    toast(`Ajuste guardado · ${delta > 0 ? '+' : ''}${delta} ud. · Stock total: ${nuevoStock}`, 'success');
    renderInventory(document.getElementById('invSearch').value);
    setStatus('ok', 'Ajuste en BD');
  }
}
