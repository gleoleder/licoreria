...
async function saveStockAdj() {
  ...
  if (tipo === 'entrada') {
    ...
    const mov = { product_id: adjProdId, product_name: p.name, date: new Date().toISOString(), type: 'entrada', qty, notes: `Lote FIFO · Bs ${costo}/u · ${nota}`, usuario: getUser().email };
    ...
    const mov = { product_id: adjProdId, product_name: p.name, date: new Date().toISOString(), type: tipo, qty: delta, notes: nota, usuario: getUser().email };
    await DB.addMovement(mov); Sheets.addMovimiento(mov);
...
function setupLoginScreen() {
  ...
  // Intento silencioso por si ya había consentido antes
