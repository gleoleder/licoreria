...
async function processSale() {
  ...
  const sale = {
    ...
    usuario:      getUser().email,
    synced:       false   // se marca true cuando Sheets confirma
  };
  lastSaleId = await DB.addSale(sale);
  sale.id    = lastSaleId;
  // Guardar en Sheets en background — si falla queda pendiente para el próximo sync
  Sheets.addVenta(sale)
    .then(() => DB.saveSale({ ...sale, synced: true }))
    .catch(() => toast('Sin conexión — venta guardada localmente', 'warn'));
  for (const item of saleItems) {
    const up = await DB.getProduct(item.product_id);
    if (up) Sheets.saveProduct(up, getUser().email);
  }
  Sheets.syncLotes();
...
async function saveStockAdj() {
  ...
  if (updProd) Sheets.saveProduct(updProd, getUser().email);
