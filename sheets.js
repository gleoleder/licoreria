...
const Sheets = {
  ...
  async _write(method, path, body) {
    if (!this._token) {
      ...
      return new Promise((resolve, reject) => {
        ...
        // Silencioso — nunca mostrar popup a un usuario ya autenticado
        this._tokenClient.requestAccessToken({ prompt: '' });
      });
    }
  ...
  async loadFromSheets(onProgress) {
    ...
    // 5. Ventas: SIEMPRE desde Sheets (fuente de verdad para multi-dispositivo)
    onProgress?.('Cargando ventas…');
    try {
      // Primero: reenviar ventas pendientes (no sincronizadas por falta de internet)
      ...
      // Borrar ventas locales y reemplazar con las de Sheets
      for (const s of localSales) await DB._delete('sales', s.id);
    ...
    } catch (_) { /* sin ventas en Sheets — mantener las locales */ }
