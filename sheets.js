// ═══════════════════════════════════════════════════════
//  POS v2 — Módulo de sincronización con base de datos
//  Lee  → API de Google Sheets v4 con API Key (sin auth)
//  Escribe → API de Google Sheets v4 con OAuth 2.0
// ═══════════════════════════════════════════════════════
...
const Sheets = {
  ...
  async checkUserAccess(email) {
    try {
      ...
      console.warn('[DB] No se pudo leer hoja Usuarios:', e);
      return false;
    }
  },
  ...
  _tokenClient:  null,
  _sheetIds:     {},
  _onIdentified: null,   // callback(userInfo) llamado tras identificar al usuario

  // Un solo popup para TODO: identidad + acceso a base de datos
  // scope incluye openid/email/profile → da el correo sin segundo popup
  initAuth(onIdentified) {
    if (!this._clientId()) {
      console.warn('[DB.initAuth] Client ID no configurado');
      return;
    }
    console.log('[DB.initAuth] Iniciando auth, onIdentified:', !!onIdentified);
    if (onIdentified) this._onIdentified = onIdentified;
    const load = () => {
      this._tokenClient = google.accounts.oauth2.initTokenClient({
        ...
        callback: async resp => {
          console.log('[DB.initAuth] Callback de token:', resp.error ? 'ERROR' : 'OK');
          if (resp.error) { console.warn('[DB Auth]', resp.error); return; }
          this._token = resp.access_token;
          console.log('[DB.initAuth] Token obtenido, length:', this._token?.length);
          this._updateAuthUI(true);
          // Si hay callback de identificación, obtener perfil y llamarlo
          if (this._onIdentified) {
            console.log('[DB.initAuth] Llamando callback _onIdentified');
            const info = await this._getUserInfo();
            const cb = this._onIdentified;
            this._onIdentified = null;
            if (cb) cb(info);
          } else {
            console.log('[DB.initAuth] Sin callback _onIdentified - token solo para escritura');
          }
        }
      });
      // Intento silencioso (no muestra popup si ya consintió)
      console.log('[DB.initAuth] Solicitando token silencioso (prompt: "")');
  ...
  // Popup explícito (para login o cuando el token expiró)
  // Primero intenta silencioso, si falla muestra popup
  requestAuth(onIdentified) {
    if (onIdentified) this._onIdentified = onIdentified;
    if (!this._tokenClient) { this.initAuth(onIdentified); return; }
    // Intentar primero sin popup
    this._tokenClient.requestAccessToken({ prompt: '' });
    // Si no hay token después de 2 segundos, intentar con popup
    setTimeout(() => {
      if (!this._token && this._tokenClient) {
        this._tokenClient.requestAccessToken({ prompt: 'select_account' });
      }
    }, 2000);
  },

  // Obtener perfil del usuario desde el token activo
  async _getUserInfo() {
  ...
  _updateAuthUI(ok) {
    ...
    if (chip && ok) chip.style.borderColor = 'var(--green)';
  },
  ...
  async _write(method, path, body) {
    if (!this._token) {
      ...
      return new Promise((resolve, reject) => {
        ...
        // Intento silencioso primero (sin popup)
        this._tokenClient.requestAccessToken({ prompt: '' });
      });
    }
    ...
    if (res.status === 401) {
      this._token = null;
      // Token expiró: reintentar con nuevo token (popup si es necesario)
      return new Promise((resolve, reject) => {
        const orig = this._tokenClient._callback;
        this._tokenClient._callback = resp => {
          if (resp.error) { reject(new Error(resp.error)); return; }
        ...
        // Intentar primero sin popup, si falla mostrará popup después de 2s
        this._tokenClient.requestAccessToken({ prompt: '' });
      });
    }
  ...
  //  Inventario:  ID | Nombre | Categoría | Unidad | Costo | Precio | Stock | Stock mín. | Barcodes | Usuario | Fecha
  //  Catálogo:    Nombre | Categoría | Unidad | Precio Bs | Costo Bs | Stock mín.
  async saveProduct(p, usuario = '') {
    console.log('[DB.saveProduct] Guardando producto:', p.name);
    const fecha = new Date().toISOString();
    // Inventario
    const invRow = [[p.id||'', p.name, p.category, p.base_unit, p.cost, p.price, p.stock, p.min_stock, JSON.stringify(p.barcodes||[]), usuario, fecha]];
    const invNum = await this._findRow('Inventario', 1, p.name);
    if (invNum > 0) {
      console.log('[DB.saveProduct] Actualizando fila', invNum, 'en Inventario');
      await this._write('PUT',
        `/values/Inventario!A${invNum}:K${invNum}?valueInputOption=USER_ENTERED`,
        { range: `Inventario!A${invNum}:K${invNum}`, majorDimension: 'ROWS', values: invRow }
      );
    } else {
      console.log('[DB.saveProduct] Creando nueva fila en Inventario');
      await this._write('POST',
        `/values/Inventario!A:K:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
        { range: 'Inventario!A:K', majorDimension: 'ROWS', values: invRow }
      );
    }
    ...
    if (catNum > 0) {
      console.log('[DB.saveProduct] Actualizando fila', catNum, 'en Catalogo');
      await this._write('PUT',
      ...
      console.log('[DB.saveProduct] Creando nueva fila en Catalogo');
      await this._write('POST',
    ...
    console.log('[DB.saveProduct] Producto guardado exitosamente');
  },
  ...
  async syncLotes() {
    ...
    console.log('[DB.syncLotes] Sincronizando', lots.length, 'lotes');
    ...
    console.log('[DB.syncLotes] Lotes sincronizados exitosamente');
  },

  // ── Ventas: solo append — col I = usuario ─────────────
  //  A: Fecha | B: Total | C: Método | D: Costo total | E: Ganancia | F: Recibido | G: Cambio | H: Items | I: Usuario
  async addVenta(s) {
    console.log('[DB.addVenta] Registrando venta #', s.id, 'total:', s.total);
    const row = [[
    ...
    await this._write('POST',
      `/values/Ventas!A:I:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
      { range: 'Ventas!A:I', majorDimension: 'ROWS', values: row }
    );
    console.log('[DB.addVenta] Venta registrada exitosamente');
  },

  // ── Movimientos: solo append — col G = usuario ────────
  //  A: Fecha | B: Producto | C: Tipo | D: Cantidad | E: Costo | F: Notas | G: Usuario
  async addMovimiento(m) {
    console.log('[DB.addMovimiento] Registrando movimiento:', m.type, 'producto:', m.product_name, 'qty:', m.qty);
    const row = [[m.date, m.product_name||'', m.type, m.qty, m.cost||0, m.notes||'', m.usuario||'']];
    await this._write('POST',
      `/values/Movimientos!A:G:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
      { range: 'Movimientos!A:G', majorDimension: 'ROWS', values: row }
    );
    console.log('[DB.addMovimiento] Movimiento registrado exitosamente');
  },
  ...
  async loadFromSheets(onProgress) {
    ...
    onProgress?.('¡Listo!');
    // Las ventas se sincronizan en background para no bloquear la UI
    this._syncVentasBackground();
  },

  // ── Sync ventas en background (no bloquea la UI) ─────
  async _syncVentasBackground() {
    try {
      const { val: vv, num: vn } = this._helpers();

      // 1. Reenviar ventas pendientes (sin internet al momento de vender)
      const localSales = await DB.getSales();
      const pendientes = localSales.filter(s => !s.synced);
      for (const s of pendientes) {
        try { await this.addVenta(s); } catch (_) { /* sigue offline */ }
      }

      // 2. Descargar ventas desde la base de datos
      const ventaRows = await this.readSheet('Ventas');

      // 3. Reemplazar ventas locales con las de la base de datos (transacción en lote)
    ...
    } catch (_) { /* sin conexión — conservar ventas locales */ }
  },
