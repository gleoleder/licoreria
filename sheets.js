...
const Sheets = {
  ...
  async loadFromSheets(onProgress) {
    ...
    // 4. Lotes FIFO — con deduplicación para evitar importar filas repetidas de Sheets
    onProgress?.('Cargando lotes FIFO…');
    try {
      const lotRows = await this.readSheet('Lotes');
      const seenUids = new Set(); // deduplicar por lot_uid
      const seenSig  = new Set(); // deduplicar por nombre+fecha+costo (lotes sin UID)
      ...
      for (const row of lotRows.slice(1).filter(r => r[1] && num(row, 5) > 0)) {
        ...
        // Saltar si ya importamos este lote (por UID o por firma)
