  ...
  try {
    ...
    for (var i = 0; i < requeridas.length; i++) {
      if (hojas.indexOf(requeridas[i]) === -1) {
        return error('Hoja no encontrada: "' + requeridas[i] + '" — hojas existentes: ' + hojas.join(', '));
      }
    }
...
function deleteLote(d) {
  ...
  if (rowNum < 0) {
    return error('Lote con ID "' + d.id + '" no encontrado en la hoja Lotes');
  }
...
function editVenta(d) {
  ...
  if (rowNum < 0) {
    return error('Venta con ID "' + d.id + '" no encontrada en la hoja Ventas');
  }
...
function deleteVenta(d) {
  ...
  var rowNum = findRowByCol(sheet, 0, String(d.id));
  if (rowNum < 0) {
    return error('Venta con ID "' + d.id + '" no encontrada');
  }
  sheet.deleteRow(rowNum);
